...
export class ProgramasHandler {
    ...
    private async crearPrograma(
        ...
        try {
            ...
            const partesObligatorias = await this.programasService.getPartesObligatorias();
            ...
            if (codigoMatch) {
                ...
                if (programas.length > 1) {
                    ...
                    for (const p of programas) {
                        const hora = p.horaInicio ? ` (${this.formatTime(p.horaInicio)})` : '';
    ...
    private async asignarParte(
        ...
        try {
            ...
            const parteEncontrada = programa.partes.find(pp =>
            ...
            let mensajeAsignaciones = resultado.asignaciones
                .map(a => `📌 ${a.parteNombre}`)
                .join('\n');
