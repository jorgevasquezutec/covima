...
export class NotificacionesHandler {
    ...
    async handle(
        ...
        try {
            ...
            const participantes = new Map<number, { nombre: string; telefono: string; partes: string[] }>();
            ...
            for (const asig of programa.asignaciones) {
                ...
                } else if ((asig as any).nombreLibre) {
                    // Nombre libre (sin usuario registrado)
                    const nombreLibre = (asig as any).nombreLibre as string;
                    const existing = participantesLibres.find(p => p.nombre === nombreLibre);
                    if (existing) {
                        existing.partes.push(asig.parte.nombre);
                    } else {
                        participantesLibres.push({ nombre: nombreLibre, partes: [asig.parte.nombre] });
                    }
                }
            }

            // Generar texto del programa
            const textoGenerado = await this.programasService.generarTexto(programa.id);
    ...
    private parseFecha(texto: string): Date | null {
        ...
        if (matchFecha) {
            ...
            let anio = matchFecha[3] ? parseInt(matchFecha[3], 10) : new Date().getFullYear();
