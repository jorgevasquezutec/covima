...
const levels = [
  { name: "Discípulo", icon: "🌱", xp: 0, color: "#9CA3AF", unlocked: true },
  { name: "Diácono", icon: "🤝", xp: 100, color: "#78716C", unlocked: true },
  { name: "Anciano", icon: "📖", xp: 400, color: "#A16207", unlocked: true },
  { name: "Levita", icon: "🎵", xp: 900, color: "#65A30D", unlocked: false },
  { name: "Sacerdote", icon: "🙏", xp: 1600, color: "#0891B2", unlocked: false },
];

export const NivelesScene: React.FC<{ screenshot?: string }> = ({ screenshot }) => {
  ...
  const barWidth = interpolate(barProgress, [0, 1], [0, 65]); // 65% of bar (260/400)

  // Current XP counter (Anciano level: 100-400, showing 260)
  const xpDisplay = Math.round(interpolate(barProgress, [0, 1], [0, 260]));
      ...
      <div className="w-full max-w-2xl">
        <div className="flex justify-between mb-3">
          <div className="flex items-center gap-2">
            ...
            <span className="text-xl font-bold text-white">Anciano</span>
          </div>
        ...
        <p className="text-center text-purple-200 mt-3 text-lg font-medium">
          ¡Te faltan {400 - xpDisplay} XP para ser Levita! 🎵
        </p>
      </div>
